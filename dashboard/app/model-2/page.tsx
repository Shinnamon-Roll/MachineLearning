...
async function getData() {
  const metricsPath = path.join(process.cwd(), 'public', 'data', 'metrics_model2.json');
  const historyPath = path.join(process.cwd(), 'public', 'data', 'training_history_model2.json');
...
export default async function Model2Page() {
  ...
  if (!metrics && !history) {
      return (
        ...
        <header className="mb-10 text-center relative z-10">
            <h1 className="text-3xl font-bold mb-2 uppercase tracking-widest border-b-4 border-foreground inline-block pb-2 text-white">
            Model 2: CustomMobileNetV2
            </h1>
            <p className="text-neutral-400 mt-2">MobileNetV2 with Transfer Learning (Binary Classification)</p>
        </header>

        <section className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-6 relative z-10">
            <div className="bg-neutral-800 p-8 rounded-full">
                <Hammer className="w-16 h-16 text-neutral-400" />
            </div>
            <div className="max-w-md space-y-2">
                <h2 className="text-2xl font-bold text-white">Model Not Trained Yet</h2>
                <p className="text-neutral-400">
                    Please run the training script in <code>model-2/train.py</code> to generate the model and metrics.
                </p>
            </div>
            
            <Card className="w-full max-w-lg mt-8 bg-neutral-900/50 border-neutral-800 backdrop-blur-md shadow-none">
                <CardHeader>
                    <CardTitle className="text-lg text-white">Architecture Details</CardTitle>
                </CardHeader>
                <CardContent className="text-left">
                    <ul className="list-disc list-inside space-y-2 text-neutral-400">
                        <li><strong>Base:</strong> MobileNetV2 (ImageNet Weights)</li>
                        <li><strong>Classifier:</strong> Custom Linear Head (2 Classes)</li>
                        <li><strong>Freezing:</strong> First 40% of layers frozen</li>
                        <li><strong>Optimization:</strong> Adam Optimizer, CrossEntropyLoss</li>
                    </ul>
                </CardContent>
            </Card>
        </section>
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-foreground p-4 md:p-8 font-sans transition-colors duration-300 relative overflow-hidden">
      ...
      <header className="mb-10 text-center relative z-10">
        ...
        <p className="text-neutral-400 mt-2 text-lg">MobileNetV2 with Transfer Learning (Binary Classification)</p>
      </header>
      ...
      <section className="mb-16 border-b border-neutral-800 pb-10 relative z-10">
        <div className="flex items-center gap-2 mb-6">
           ...
           <h2 className="text-2xl font-bold text-white">Try Model 2</h2>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-neutral-300">
                <div className="space-y-2">
                    ...
                    <div className="flex justify-between border-b border-neutral-800 pb-1">
                        ...
                        <span className="text-neutral-400">{metrics?.classes.join(", ") || "Loading..."}</span>
                    </div>
                    <div className="flex justify-between border-b border-neutral-800 pb-1">
                        <span className="font-semibold text-white">Test Samples</span>
                        <span className="text-neutral-400">{metrics?.test_samples || "N/A"}</span>
                    </div>
                </div>
                <div className="md:col-span-2 bg-neutral-800/50 p-4 border border-neutral-700 rounded-md">
                  <h4 className="font-bold mb-2 text-white">Preprocessing Steps:</h4>
                  <ul className="list-disc list-inside text-sm text-neutral-400 space-y-1">
                    ...
                    <li>Data Augmentation (RandomResizedCrop, Rotation)</li>
                  </ul>
                </div>
              </div>
            </CardContent>
